...
namespace AutoHotKeyTrigger.ProfileManager
{
    ...
    public class Rule
    {
        ...
        {
            var rules = new Rule[4];
            for (var i = 0; i < 2; i++)
            {
                rules[i] = new($"SmallLifeFlask{i + 1}");
                rules[i].Enabled = true;
                rules[i].Key = VK.KEY_1 + i;
                rules[i].conditions.Add(new DynamicCondition(
                    $"PlayerVitals.HP.Percent <= 70 && Flasks.Flask{i + 1}.IsUsable && !Flasks.Flask1.Active && !Flasks.Flask2.Active"));
            }

            rules[2] = new($"QuickSilverFlask4");
            rules[2].Enabled = false;
            rules[2].Key = VK.KEY_4;
            rules[2].conditions.Add(new DynamicCondition($"PlayerAnimation.Equals(Animation.Run)", new Wait(1)));
            rules[2].conditions.Add(new DynamicCondition($"Flasks.Flask4.IsUsable && !Flasks.Flask4.Active"));

            rules[3] = new($"SmallManaFlask5");
            rules[3].Enabled = true;
            rules[3].Key = VK.KEY_5;
            rules[3].conditions.Add(new DynamicCondition($"PlayerVitals.MANA.Percent <= 20 && Flasks.Flask5.IsUsable && !Flasks.Flask5.Active"));
