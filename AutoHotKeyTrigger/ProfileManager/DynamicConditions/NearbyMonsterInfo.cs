        ...
        public NearbyMonsterInfo(InGameState state)
        {
            foreach (var entity in state.CurrentAreaInstance.AwakeEntities.Values)
            {
                ...
                if (entity.EntityType != EntityTypes.Monster ||
                    entity.EntityState == EntityStates.LegionStage0 ||
                    entity.EntityState == EntityStates.LegionStage1Dead ||
                    entity.IsOrWasMonsterSubType(EntitySubtypes.HarbingerMonster) ||
                    entity.IsOrWasMonsterSubType(EntitySubtypes.TormentedSpiritsMonster) ||
